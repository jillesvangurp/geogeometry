...
- Shared library code is primarily in `src/commonMain/kotlin` (`com.jillesvangurp.geo` and `com.jillesvangurp.geojson`), with shared tests in `src/commonTest/kotlin`.
...
1. Multiplatform stays intact
   ...
   - Keep build/target configuration aligned with `build.gradle.kts`.
